...
string lastfile;
string firstfile;
int receiveNum = 0;//记录已从客户端那里接受到的文件数
...
void Server::__start(int argc,char* argv[])
{
    int len;
    initialization();
	try {
	...
	while (_access(file_path.c_str(), 0) == -1)	//如果文件夹不存在
	{
		cout << "不存在该文件夹或者文件,请重新输入\n请输入【文件地址】 【ip】 【端口号】中间以一个空格隔开" << endl;
		cin >> file_path >> server_ip >> port;
	}
    cout << "服务器已经启动" << endl;
...
void Server::__trans(int &flag)
{
	...
	{	
		...
		ss << s;
		ss >> receiveNum;
		cout << "上传一次传输到第" << receiveNum << "个文件" << endl;
	}
	

	//检查上次传输是否正常结束
	bool normalendOr = true;//为true表明正常结束，否则被中断
	fin1.open("record.txt");
	//如果能打开，且内容不为Finished，则表明上次传输被中断
	if (fin1)
	{
		string s;
		getline(fin1, s);
	...
	//如果上次传输被中断，则把传输好的文件个数发送给客户端
	//否则发送0，表示从头开始传
	if (normalendOr)
	{
		if (send(s_accept, "0", 1, 0) < 0)
		{
			cout << "程序出错006" << endl;
			re__start();
			return;
	...
	{
		if (send(s_accept, to_string(receiveNum).c_str(), to_string(receiveNum).length(), 0) < 0)
		{
			cout << "程序出错006" << endl;
			re__start();
			return;
   ...
   // thread t_2(executive);
    ...
    cout << "接收完毕" << endl;
	//接收完毕，记录接收完毕信息并将接受文件数目恢复为0
	fout.open("record.txt");
	fout << "Finished";
	fout.close();
	fout.open("ceivedNum.txt");
	fout << "0";
	fout.close();
