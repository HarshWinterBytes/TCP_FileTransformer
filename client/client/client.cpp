...
int serverFileNum = 0;//定义服务端已接收好的文件数
...
bool Client::sent_end_transmission()
{
	...
	{
		cout << "出错了,结束传输指令没有正常收到回复" << endl;
		return false;
	}
	else
	{
		cout << "确认服务器已完成传输" << endl;
	}
	return true;
}
...
void Client::__connect(int argc, char* argv[])
{
	int recv_len;
	char recv_buf[100];
	initialization();
	...
	*/
	...
	if (count != 1)
	{
		...
		else
		{
			...
			while (!fin.eof())
			{
				...
				filesCount++;
				if (filesCount > serverFileNum)break;
			}
			sendedPath.pop_back();
			filesCount = 0;
			...
			if (struct_diff == 0)
			{
				...
				while (times.size() > sendedfiles.size())
					times.pop_back();
				
				for (int i = 0;i < sendedfiles.size() && i < times.size();i++)
				{
	...
	if (count != 1)
	{
		...
		else
		{
			...
			cout << "检测到上次传输被中断，且已传输部分没有变动，将从" << files[serverFileNum] << "开始传输" << endl << endl;
		}
	}

	for (int i = serverFileNum; i < files.size(); i++)
	{
		if (is_file_packet[i])
		{
